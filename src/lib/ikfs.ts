...
export const ChunkFns = (fsRead: FSReadFn, fsWrite: FSWriteFn) => {
    ...
    const writeLength = async (fd: number, length: number, filePosition: number) : Promise<void> => {
        const lengthStr = `${length}`.padStart(20, '0');
        const buffer = Buffer.from(lengthStr);
        await writeBuffer(fd, buffer, {offset: 0, length: buffer.length}, filePosition);
    };
    const writeContinuation = async (fd: number, continuation: number, filePosition: number) : Promise<void> => {
    ...
    const writeToSpace = async (
        ...
        const bytesWritten = await writeBuffer(fd, buffer, bufferSpan, fileContentSpan.offset);
        ...
        if (bufferRemainingLength === 0) {
            ...
            writeContinuation(fd, continuation, continuationFilePosition);
        }
        return bytesWritten;
    };
    return {
...
export type FSOpenFn = (path: string, flags: string, callback: (err: NodeJS.ErrnoException|null, fd: number) => void) => void;
export const OpenForReadingFn = (fsOpen: FSOpenFn) => (filepath: string) => open(fsOpen, filepath, 'r');
export const OpenForWritingFn = (fsOpen: FSOpenFn) => (filepath: string) => open(fsOpen, filepath, 'w');
const open = (fsOpen: FSOpenFn, filepath: string, mode: string) : Promise<number> =>
...
export type FSCloseFn = (fd: number, callback: (err: NodeJS.ErrnoException|null) => void) => void;
...
export type FSStatsFn = (path: string, callback: (err: NodeJS.ErrnoException|null, stats: Stats) => void) => void;
export const StatsFn = (fsStats: FSStatsFn) => (filepath: string) : Promise<Stats> =>
...
export const SizeFn = (fsStats: FSStatsFn) => async (filepath: string) : Promise<number> => {
    const stats = StatsFn(fsStats);
    const s = await stats(filepath);
    return s.size;
};

export type FSTruncateFn = (path: string, len: number|undefined|null, callback: (err: NodeJS.ErrnoException|null) => void) => void;
export const TruncateFn = (fsTruncate: FSTruncateFn) => (filepath: string, size: number) : Promise<void> =>
    new Promise<void>((resolve, reject) => {
...
export const ReadFn = (fsRead: FSReadFn) => (fd: number, fileSpan: Span) : Promise<Buffer> =>
...
export type FSWriteFn = (
    ...
    callback: (err: NodeJS.ErrnoException | null, written: number, buffer: Buffer) => void,
) => void;
export const WriteFn = (fsWrite: FSWriteFn) => (fd: number, buffer: Buffer, bufferSpan: Span, filePosition: number) : Promise<number> =>
    new Promise<number>((resolve, reject) => {
        fsWrite(fd, buffer, bufferSpan.offset, bufferSpan.length, filePosition, (err, bytesWritten) => {
            if (err) reject(err);
